...
App.controller('fileUploadDemoCtrl',['$scope','$rootScope',function($scope,$rootScope){
    ...
    $scope.fileUploadOption = {
        ...
        getWidgetApi: function (widgetApi) {
            ...
            $scope.widgetApi.bindEvent('Complate',function(file){
                //alert();
                $("#downloadLink").attr("href",$rootScope.appPath+"whhJsPlatform/pages/demoFileUpload/"+file.name);
                $("#downloadLink").text($rootScope.appPath+"whhJsPlatform/pages/demoFileUpload/"+file.name);
            });
